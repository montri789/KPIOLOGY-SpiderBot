	...
	<table class="table table-bordered table-striped tablesorter" id="sort" border="0" cellspacing="5" cellpadding="5">
	  ...
	  <?PHP
		...
		foreach($rp as $row){
		    ...
		    /*
		    //empty($row["match_complete"])? "" : $row["match_complete"];
		    $cal_percen= round(($row["match_complete"]/$row["count_subject"])*100,2);		    
		    $match_complete= empty($row["match_complete"])? "" : $row["match_complete"]." / ( ".$cal_percen."% )";
		   		    
		    $percen_color ="default";
		    
		    if($cal_percen == 100){
			$percen_color ="green";
		    }else if($cal_percen >= 70){
			$percen_color ="green2";
		    }else if($cal_percen >= 50){
			$percen_color ="green3";
		    }else{
			$percen_color ="red1";
		    } */
	  ...
	  <tr id="id_<?=$row["client_id"]?>">
	    ...
	    <td><a href="javascript:void(0);" id="<?=$row["client_id"];?>" class="chart"><?=$row["client_name"];?></a>
	    ...
	    <td style="text-align: center;"><?=$row["count_subject"];?></td>
	     <!--<td style="text-align: left;" class="<?=$percen_color;?>"><?=$match_complete;?></td>
	    <td style="text-align: center;" class="font_green"><?=$match_complete;?></td>
...
<input type="hidden" value="34" id="client_id" /> 
...
<script type="text/javascript">
	...
	$(function(){
		...
		function updateData(){				
			...
			var url = '<?=base_url();?>index.php/match/updateData';
		...
		function loadData(){				
			...
			var url = '<?=base_url();?>index.php/match/getData';
