...
class Report extends CI_Controller {
	...
	public function getData(){
		...
		$sql ="select c.client_name,s.subject,s.id as subject_id,sm.*
				,(select max(match_to) from status_match
					where client_id=c.client_id and subject_id=s.id) as match_to
			from clients c
			inner join subject s on s.client_id=c.client_id
			left join status_match sm on sm.subject_id=s.id and sm.client_id=c.client_id 
			and sm.match_to=(select max(match_to) from status_match where client_id=c.client_id)
