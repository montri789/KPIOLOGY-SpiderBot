...
class Login_other extends CI_Controller 
{	
    	...
    	try 
    	{
    		...
    		else 
    		{
    			...
    			if($lightopenid->validate())
    			{
                    ...
                    $contact_email = $lightopenid->data['openid_ext1_value_contact_email'];
                    $namePerson_first = $lightopenid->data['openid_ext1_value_namePerson_first'];
                    $namePerson_last = $lightopenid->data['openid_ext1_value_namePerson_last'];
                    
                    $domain = explode('@',$contact_email);
                    
                    if(!isset($domain[1]) || isset($domain[1])!='thothmedia.com'){
                        echo "<script>alert('Please Login @thothmedia.com');</script>";
                        redirect('login', 'refresh');
                    }else{
                        
                        $data['email'] = $contact_email;
                        $data['first'] = $namePerson_first;
                        $data['last'] = $namePerson_last;
                    ...
                    (array('google_open_id' => $google_open_id), 'google_open_id');*/
					...
					{
						// simulate what happens in the tank auth
						echo $this->session->set_userdata(array(	'user_id' => $user[0]->id, 'username' => $user[0]->username,
													'status' => ($user[0]->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED));
						//$this->tank_auth->clear_login_attempts($user[0]->email); can't run this when doing twitter
