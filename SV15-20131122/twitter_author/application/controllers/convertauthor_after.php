...
class ConvertAuthor_after extends CI_Controller {
	...
	public function move_all_tweets()
	{
		...
		$table_row_count = $this->db->count_all('post_twitter_after');
		...
		while(($offset==0) || ($size==$load) && ($offset < $max))
		{
			...
			//	echo "\n query ";
			//$query = $this->db->get_where('post_twitter_after',array('author_id'=>0),$load,$offset);
			$query = $this->db->get('post_twitter_after',$load,$offset);
			$tweets = $query->result();
			$size = count($tweets);
			$offset+= $size;
					...
					if(preg_match('/[\x{0E00}-\x{0E7F}]/u', $t->body) || preg_match('/[A-Za-z0-9_~\-!@#\$%\^&\*\(\)]/', $t->body)){				
					 ...
					 if($pass < 1){
						...
						echo "D";
					       ...
					       $this->db->delete('post_twitter_after');
					}
			}
		}
		...
		echo 'OK'."\n";
		//Delete post_twitter_after
		echo "Delete Twitter Temp Table:";

		$res3 = $this->db->where_in( id, $tweet_id );
		$res2 = $this->db->delete('post_twitter_after');

		/*
			//$res2 = $this->db->delete('post_twitter_after', array('id' => $id)); 
			//$res2 = $this->db->update('post_twitter_after', $data2, array('id' => $tweet_id));
			if($res2) echo 'OK'."\n";
			else	echo 'FAILED'."\n";
