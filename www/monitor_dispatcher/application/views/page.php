...
<div class="row" id="page">
	...
	<table class="table table-bordered table-striped tablesorter" id="sort" border="0" cellspacing="5" cellpadding="5">
		...
		<tbody><?PHP foreach($website as $row){ ?>
			<tr id="id-<?=$row["id"];?>">
				<td class="website_id"><?=$row["id"];?></td>
				<td class="website_name" ><?=$row["name"];?></td>
...
<script type="text/javascript">
	$(function(){
		...
		function loadDate(){
			...
			var url = '<?=base_url();?>index.php/page/website';
					...
					if(res != ''){
						$.each(res,function(index){
							...
							if(row.insert_date != null){
								var insert_date  = row.insert_date.split(' ');
							...
							$('#id-'+row.id+' td.last_insert_date').text(row.insert_date);
		...
		function update(){
			var url  = '<?=base_url();?>index.php/page/update';
