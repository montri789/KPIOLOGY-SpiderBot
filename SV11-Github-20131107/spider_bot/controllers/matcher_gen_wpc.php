...
class Matcher_gen_wpc extends CI_Controller{
	...
	public function bot($type='queue',$client_id=null,$to_date=null)
	{
		...
		$bot_query = $this->db->get_where('subject_wpc',array('bot_id'=>$bot_id));
		...
		$query = $this->db->get_where('subject_wpc',$option,1,0);
	...
	public function run_all($type='queue',$client_id=null,$from=null,$to=null)
	{
		...
		$query = $this->db->get_where('subject_wpc',$option);
	...
	public function run($subject_id,$clean=true,$query_offset=0,$from,$to)
	{
		...
		$subject_data = $this->custom_model->get_multi_value('subject_wpc','client_id,query,matching_status',$subject_id);
		...
		$this->db->update('subject_wpc',array('matching_status'=>'matching'),array('id'=>$subject_id));
...
//					echo '('.$k.')'.$docinfo["id"]." ";
					...
					//{
						...
						if($post->type == "post" || $post->type == "comment"){	
							...
							if($postData != null){
								...
								$sql_filename ="gen_sqlFile/c_id_".$subject->client_id."-s_id_".$subject_id."-w.sql";
							...
							if($postData != null){
								...
								$sql_filename ="gen_sqlFile/c_id_".$subject->client_id."-s_id_".$subject_id."-t.sql";
							...
							if($postData != null){
								...
								$sql_filename ="gen_sqlFile/c_id_".$subject->client_id."-s_id_".$subject_id."-f.sql";
			...
			foreach($q_post_id_from_list_page_id->result_array() as $val){
						...
						$sql_filename ="gen_sqlFile/c_id_".$client_id."-s_id_".$subject_id."-wpc.sql";
...
//						if($res) $insert_website_post_comment[] = $data;
		...
		if($check == true){
			...
			$this->db->update('subject_wpc',$data,array('id'=>$subject_id));
		}
	...
	public function update($subject_id=null)
	{
		...
		if($query->num_rows())
		{
			foreach($query->result() as $row)
			{
				...
				// flag subject as matching.. do other bot runs this queue.
				$this->db->update('subject_wpc',array('matching_status'=>'matching'),array('id'=>$row->id));
				...
				// flag subject as update..
				$this->db->update('subject_wpc',array('matching_status'=>'update','latest_matching'=> mdate('%Y-%m-%d %H:%i:%s',time())),array('id'=>$row->id));
	...
	public function run_queue()
	{
		...
		$query = $this->db->get_where('subject_wpc',$options);
	...
	function eval_sentiment($str,$subject_id,$debug=0)
	{
		...
		$query = $this->db->get_where('subject_wpc',array('id'=>$subject_id));
