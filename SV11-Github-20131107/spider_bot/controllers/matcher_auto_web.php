...
class Matcher_auto_web extends CI_Controller{

	public function bot($type='queue',$client_id=null,$to_date=null)
	{
		//$this->clear($client_id);
	
		$query_index = $this->db->get_where('sphinx_index',array('name'=>'27.254.81.11.9313'));
		$match_to_id =$query_index->row()->match_1;
		echo 'match_to_id:'.$match_to_id.PHP_EOL;
		
		//if($to_date==null) $to_date = mdate('%Y-%m-%d',time());
		//echo 'to date:'.$to_date.PHP_EOL;
		//echo 'to date:'.$to_date.PHP_EOL; exit;
         ...
         */
			...
			if(!$subject_taken)
			{
				...
				//$res = $this->$subject_id,$clean,0,$from_date,$to_date);
				$res = $this->run($subject_id,$clean,0,$match_to_id);
	...
	public function run($subject_id,$clean=true,$query_offset=0,$match_to_id=0)
	{
		...
		*/
		...
		else
		{
			...
			else if($res['total'] > $limit+$offset) $this->run($subject_id,$clean,$limit+$offset,$match_to_id);
			else
			{
...
//					echo '('.$k.')'.$docinfo["id"]." ";
	...
	public function clear()
	{
		...
		echo "Clear bot (-1) = ".$this->db->affected_rows()." Row".PHP_EOL;
