...
class Test extends CI_Controller {
	...
	public function pantip_insert_page($start = null,$end = null){	
		...
		$config_wr['hostname'] = "203.151.21.106";
		...
		$sql = "SELECT url FROM page WHERE domain_id = 212 ORDER BY ID DESC LIMIT 1";
		...
		if($start > $url || empty($url)){
			...
			$connect_kpio->insert_batch('page',$insert);	//insert to kpiology
			echo "\n------success insert to kpiology------\n";
			$connect_wr->insert_batch('page',$insert);	//insert to warroom
			echo "------success insert to warroom-------\n";
			*/
	...
	public function delete_post($client_id = null,$month = null,$year = 2012,$type = "website")
	{	
				
		$thothconnect_db = $this->connect_db();
			
			
		echo "CLIENT : ".$client_id." MONTH : ".$month." YEAR :".$year." TYPE : ".$type."\n";
		echo ".............................................\n";
		
		$sql = "SELECT ";
		
		if($client_id == 3 || $client_id == 7 || $client_id == 21 || $client_id == 22 || $client_id == 23 || $client_id == 27 || $type == "twitter" || $type == "facebook"){
			$sql .= "id,";
		}
		
		$sql .= " post_id,subject_id
			FROM ".$type."_c".$client_id."
			WHERE MONTH(post_date) = $month AND YEAR(post_date) = $year 
			GROUP BY subject_id,body HAVING count(post_date) > 1 ";
		...
		if($query->num_rows() > 0)
		{
			foreach($query->result_array() as $row)
			{
				...
				if($client_id == 3 || $client_id == 7 || $client_id == 21 || $client_id == 22 || $client_id == 23 || $client_id == 27 || $type == "twitter" || $type == "facebook"){
					...
					echo "Delete  ID : ".$row["id"]."\n";
					
					$sqls = "DELETE FROM ".$type."_c".$client_id." WHERE id = ".$row["id"];
							
				}else{
					
				echo "Delete POST ID : ".$row["post_id"]." SUBJECT ID :".$row["subject_id"]."\n";
				$sqls = "DELETE FROM ".$type."_c".$client_id." WHERE post_id = ".$row["post_id"]." AND subject_id = ".$row["subject_id"];
			...
			$this->delete_post($client_id,$month,$year,$type);
		}
		
		$this->update_match_repeat($client_id,$month,$year);
	}
	
	public function delete_post_new($client_id = null,$month = null,$year = 2012,$type = "website")
	{			
		$thothconnect_db  = $this->connect_db();
		...
		if($query->num_rows() > 0)
		{
			...
			echo "------------------------------\n";
			$this->delete_post_new($client_id,$month,$year,$type);
		}
		
		$this->update_match_repeat($client_id,$month,$year);
	}
	
	function update_match_repeat($client_id,$month,$year){
		
		
		$data = array();
		$data["client_id"] = $client_id;
		$data["date"] = date('Y-m-t',strtotime("$year-$month")); 
				
		$insert_query = $this->db->insert_string("match_repeat",$data);
		$insert_query = str_replace('INSERT INTO','INSERT IGNORE INTO',$insert_query);
		$this->db->query($insert_query);
		
		echo "------------------------\n";
		echo "insert match repeat\n";
	}
	...
	{	
		...
		$sql = "SELECT id,post_id,subject_id  
				FROM website_c$client_id 
				WHERE (MONTH(post_date) = 11 AND YEAR(post_date) = 2012)   
				GROUP BY post_id,subject_id 
				HAVING COUNT(id) > 1  ";
		...
		if($query->num_rows() > 0)
		{
			...
			{
				set_time_limit(3);
				echo "Delete ID : ".$row["id"]." POST ID : ".$row["post_id"]." SUBJECT ID :".$row["subject_id"]."\n";
							
				///$thothconnect_db->reconnect();
							
				$sqls = "DELETE FROM website_c".$client_id." WHERE id = ".$row["id"]."  ";
				$thothconnect_db->query($sqls);
			}
			echo "------------------------------\n";
			$this->deletepage();
...
//		$to_date = new Datetime('2012-06-01');
	...
	public function search()
	{
		...
		if ( $limit )				$cl->SetLimits ( $offset, $limit, ( $limit>10000 ) ? $limit : 10000 );
		$cl->SetRankingMode ( $ranker );

		//$cl->SetFilterRange('post_date',strtotime('2012-11-20 00:00:00'),strtotime('2012-11-30 23:59:59'));
