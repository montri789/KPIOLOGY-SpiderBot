...
class Fetch3_pantip_not_insert extends CI_Controller {
	...
	public function all($domain_id=null,$limit=null,$offset=null,$new='new')
	{
		...
		//$condition = ' AND MONTH(insert_date)=MONTH(CURDATE()) AND YEAR(insert_date)=YEAR(CURDATE()) AND id NOT IN(SELECT page_id FROM post)';
		$condition = ' AND insert_date BETWEEN \''.$start_date.'\' AND \''.$end_date.'\' AND id NOT IN(SELECT page_id FROM post)';
		...
		if($domain_id != null)
		{
			...
			$condition .=' ORDER BY insert_date desc,active_score desc';
		
		if($offset!=null && $limit!=null)
		{
			echo "LIMITE = $limit, OFFSET = $offset\n";
			$condition .=' LIMITE = $limit, OFFSET = $offset';
		}
