...
class Fetch3_test extends CI_Controller {
	...
	public function all($domain_id=null,$limit=null,$offset=null,$new='new')
	{
		...
		//not check web Pantip fetch all
		if($domain_id !=212){
			if($new=='new') $option['parse_post'] = 0;
			else if($new=='old') $option['parse_post !='] = 0;
		...
		$this->db->order_by('insert_date desc,active_score desc');
		$query = $this->db->get_where('page',$option);
		...
		if($query->num_rows() > 0)
		{
			...
			foreach($query->result() as $row)
			{
				...
				if($res != null)
				{
					if(!$res['parse_ok']){
						$page->parse_post = -1;
//sent email for alert parse failed		//$this->load->model("sent_email_model");
	...
	public function pantipnew_insert_page($start = null,$end = null){	
		...
		if($start > $url || empty($url)){
			...
			for($i = $start; $i<= $end; $i++){
				...
				$site='http://pantip.com/topic/30903535';
				$site='http://pantip.com/topic/30905206';
				
				$contents =file_get_contents($site);
				//$text_position =strpos($contents,'ไม่พบหน้าที่คุณต้องการ');
				$text_position =strpos($contents,'callback-status');
